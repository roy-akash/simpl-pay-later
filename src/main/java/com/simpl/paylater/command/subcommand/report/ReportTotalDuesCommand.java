...
@Service
public class ReportTotalDuesCommand
        implements ICommand
{
    ...
    {
        if(args.length < 2){
            throw new PayLaterException("Incomplete arguments for total dues for all users command");
        }
    }
    ...
    {
        List<User> users = reportService.totalDuesForAllUsers();
        StringBuilder result = new StringBuilder();

        users.forEach(user -> result.append(user.getName()).append(": ").append(user.getDues()).append(END_LINE));
