...
import com.simpl.paylater.command.subcommand.payback.PayBackCommand;
...
public class PayBackCommandClassifier
{

    @Autowired
    PayBackCommand payBackCommand;
    ...
    public ICommand getRequiredCommand(String... args){
        ...
        PayBackCommandType payBackCommandType = PayBackCommandType.fromName(args[0]);
        ICommand command;
        switch (payBackCommandType) {
            case PAYBACK:
                command = payBackCommand;
                break;
            default:
                throw new PayLaterException(String.format("Invalid Command : %s ", payBackCommandType.getName()));
        }
        return command;
    }
